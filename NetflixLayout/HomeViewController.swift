...
class HomeViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        let netflixLogo = #imageLiteral(resourceName: "netflix_icon")
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: netflixLogo, style: .plain, target: nil, action: nil)
    ...
    private func createSectionHeader() -> NSCollectionLayoutBoundarySupplementaryItem {
        // Section Header Size
        let sectionHeaderSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .absolute(30))
...
extension HomeViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if kind == UICollectionView.elementKindSectionHeader {
            guard let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "ContentCollectionViewHeader", for: indexPath) as? ContentCollectionViewHeader else { fatalError("Could not dequeue header")}
            
            header.sectionName.text = contents[indexPath.section].sectionName
            return header
        }else {
